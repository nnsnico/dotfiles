...
target_bundle_identifiers = ["com.parallels.desktop.console"]

# `~/.config/karabiner/assets/complex_modifications/`内の変更したいファイルを選択
target_json = "1531129043.json"
...
def main():
    escaped_bundle_identifiers = list(
        map(lambda x: karabiner_escape(x), target_bundle_identifiers)
    )
    json = open_json()
